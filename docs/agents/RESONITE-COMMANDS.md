...
| `/resonite search` `wiki` `query` [`visible`] [`preview_chars`] | `preview_chars` **500–1500** (default **500**). Exact title or pick-menu choice → Components v2 preview: Infobox/`[[File:…]]` image in a media gallery when present; prose excerpt; rendered **wikitable** sections as monospace code blocks when the page uses `{{Table …}}` or `{|` tables (extra parse API call). Otherwise opensearch up to **10** hits; one hit → auto-load or single-option select; two or more → list + string select. |
| `/resonite search` `account` `username` [`visible`] | Public user list (`GET …/users?name=…`). Autocomplete from API. |
| `/resonite search` `github` [`visible`] | Components v2 dashboard for Yellow-Dog-Man project boards and Resonite issue repositories. **Boards** scope: optional query, pick **All boards** or a member board, then **Run** for a paginated list (Status filter when applicable, string select for issue detail with body + images when the issue is on a team board). **Repo** scope: repository, query, author, label paging (25 labels per page), issue results pagination, string select for issue detail (REST body when not on a project board). Requires `GITHUB_TOKEN`. |
